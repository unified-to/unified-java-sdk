# PatchAdsCreative2QueryParamFields

## Example Usage

```java
import to.unified.unified_java_sdk.models.operations.PatchAdsCreative2QueryParamFields;

PatchAdsCreative2QueryParamFields value = PatchAdsCreative2QueryParamFields.ID;
```


## Values

| Name                           | Value                          |
| ------------------------------ | ------------------------------ |
| `ID`                           | id                             |
| `CREATED_AT`                   | created_at                     |
| `UPDATED_AT`                   | updated_at                     |
| `NAME`                         | name                           |
| `CAMPAIGN_ID`                  | campaign_id                    |
| `GROUP_ID`                     | group_id                       |
| `ITEM_ID`                      | item_id                        |
| `ORGANIZATION_ID`              | organization_id                |
| `STATUS`                       | status                         |
| `CREATIVE_TYPE`                | creative_type                  |
| `HOSTING_SOURCE`               | hosting_source                 |
| `WIDTH`                        | width                          |
| `HEIGHT`                       | height                         |
| `ASSET_URLS`                   | asset_urls                     |
| `LINK_URL`                     | link_url                       |
| `BODY`                         | body                           |
| `TITLE`                        | title                          |
| `CTA`                          | cta                            |
| `THIRD_PARTY_TAG`              | third_party_tag                |
| `VAST_TAG_URL`                 | vast_tag_url                   |
| `EXTERNAL_CREATIVE_REFERENCE`  | external_creative_reference    |
| `EXTERNAL_PLACEMENT_REFERENCE` | external_placement_reference   |
| `EXTERNAL_AD_REFERENCE`        | external_ad_reference          |
| `PROMOTED`                     | promoted                       |
| `PATH1`                        | path1                          |
| `PATH2`                        | path2                          |
| `DATA`                         | data                           |
| `RAW`                          | raw                            |