/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package to.unified.unified_java_sdk.models.shared;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.annotation.Nullable;
import java.lang.Override;
import java.lang.String;
import java.util.List;
import java.util.Optional;
import to.unified.unified_java_sdk.utils.Utils;


public class PropertyAdsReportMetricsCampaignTargetingAudience {

    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("behaviors")
    private List<AudienceSegment> behaviors;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("combination_spec")
    private List<AudienceCombination> combinationSpec;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("custom_audiences")
    private List<AudienceSegment> customAudiences;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("excluded_behaviors")
    private List<AudienceSegment> excludedBehaviors;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("excluded_custom_audiences")
    private List<AudienceSegment> excludedCustomAudiences;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("excluded_interests")
    private List<AudienceSegment> excludedInterests;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("interests")
    private List<AudienceSegment> interests;


    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("lookalike_audiences")
    private List<LookalikeAudience> lookalikeAudiences;

    @JsonCreator
    public PropertyAdsReportMetricsCampaignTargetingAudience(
            @JsonProperty("behaviors") @Nullable List<AudienceSegment> behaviors,
            @JsonProperty("combination_spec") @Nullable List<AudienceCombination> combinationSpec,
            @JsonProperty("custom_audiences") @Nullable List<AudienceSegment> customAudiences,
            @JsonProperty("excluded_behaviors") @Nullable List<AudienceSegment> excludedBehaviors,
            @JsonProperty("excluded_custom_audiences") @Nullable List<AudienceSegment> excludedCustomAudiences,
            @JsonProperty("excluded_interests") @Nullable List<AudienceSegment> excludedInterests,
            @JsonProperty("interests") @Nullable List<AudienceSegment> interests,
            @JsonProperty("lookalike_audiences") @Nullable List<LookalikeAudience> lookalikeAudiences) {
        this.behaviors = behaviors;
        this.combinationSpec = combinationSpec;
        this.customAudiences = customAudiences;
        this.excludedBehaviors = excludedBehaviors;
        this.excludedCustomAudiences = excludedCustomAudiences;
        this.excludedInterests = excludedInterests;
        this.interests = interests;
        this.lookalikeAudiences = lookalikeAudiences;
    }
    
    public PropertyAdsReportMetricsCampaignTargetingAudience() {
        this(null, null, null,
            null, null, null,
            null, null);
    }

    public Optional<List<AudienceSegment>> behaviors() {
        return Optional.ofNullable(this.behaviors);
    }

    public Optional<List<AudienceCombination>> combinationSpec() {
        return Optional.ofNullable(this.combinationSpec);
    }

    public Optional<List<AudienceSegment>> customAudiences() {
        return Optional.ofNullable(this.customAudiences);
    }

    public Optional<List<AudienceSegment>> excludedBehaviors() {
        return Optional.ofNullable(this.excludedBehaviors);
    }

    public Optional<List<AudienceSegment>> excludedCustomAudiences() {
        return Optional.ofNullable(this.excludedCustomAudiences);
    }

    public Optional<List<AudienceSegment>> excludedInterests() {
        return Optional.ofNullable(this.excludedInterests);
    }

    public Optional<List<AudienceSegment>> interests() {
        return Optional.ofNullable(this.interests);
    }

    public Optional<List<LookalikeAudience>> lookalikeAudiences() {
        return Optional.ofNullable(this.lookalikeAudiences);
    }

    public static Builder builder() {
        return new Builder();
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withBehaviors(@Nullable List<AudienceSegment> behaviors) {
        this.behaviors = behaviors;
        return this;
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withCombinationSpec(@Nullable List<AudienceCombination> combinationSpec) {
        this.combinationSpec = combinationSpec;
        return this;
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withCustomAudiences(@Nullable List<AudienceSegment> customAudiences) {
        this.customAudiences = customAudiences;
        return this;
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withExcludedBehaviors(@Nullable List<AudienceSegment> excludedBehaviors) {
        this.excludedBehaviors = excludedBehaviors;
        return this;
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withExcludedCustomAudiences(@Nullable List<AudienceSegment> excludedCustomAudiences) {
        this.excludedCustomAudiences = excludedCustomAudiences;
        return this;
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withExcludedInterests(@Nullable List<AudienceSegment> excludedInterests) {
        this.excludedInterests = excludedInterests;
        return this;
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withInterests(@Nullable List<AudienceSegment> interests) {
        this.interests = interests;
        return this;
    }


    public PropertyAdsReportMetricsCampaignTargetingAudience withLookalikeAudiences(@Nullable List<LookalikeAudience> lookalikeAudiences) {
        this.lookalikeAudiences = lookalikeAudiences;
        return this;
    }


    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        PropertyAdsReportMetricsCampaignTargetingAudience other = (PropertyAdsReportMetricsCampaignTargetingAudience) o;
        return 
            Utils.enhancedDeepEquals(this.behaviors, other.behaviors) &&
            Utils.enhancedDeepEquals(this.combinationSpec, other.combinationSpec) &&
            Utils.enhancedDeepEquals(this.customAudiences, other.customAudiences) &&
            Utils.enhancedDeepEquals(this.excludedBehaviors, other.excludedBehaviors) &&
            Utils.enhancedDeepEquals(this.excludedCustomAudiences, other.excludedCustomAudiences) &&
            Utils.enhancedDeepEquals(this.excludedInterests, other.excludedInterests) &&
            Utils.enhancedDeepEquals(this.interests, other.interests) &&
            Utils.enhancedDeepEquals(this.lookalikeAudiences, other.lookalikeAudiences);
    }
    
    @Override
    public int hashCode() {
        return Utils.enhancedHash(
            behaviors, combinationSpec, customAudiences,
            excludedBehaviors, excludedCustomAudiences, excludedInterests,
            interests, lookalikeAudiences);
    }
    
    @Override
    public String toString() {
        return Utils.toString(PropertyAdsReportMetricsCampaignTargetingAudience.class,
                "behaviors", behaviors,
                "combinationSpec", combinationSpec,
                "customAudiences", customAudiences,
                "excludedBehaviors", excludedBehaviors,
                "excludedCustomAudiences", excludedCustomAudiences,
                "excludedInterests", excludedInterests,
                "interests", interests,
                "lookalikeAudiences", lookalikeAudiences);
    }

    @SuppressWarnings("UnusedReturnValue")
    public final static class Builder {

        private List<AudienceSegment> behaviors;

        private List<AudienceCombination> combinationSpec;

        private List<AudienceSegment> customAudiences;

        private List<AudienceSegment> excludedBehaviors;

        private List<AudienceSegment> excludedCustomAudiences;

        private List<AudienceSegment> excludedInterests;

        private List<AudienceSegment> interests;

        private List<LookalikeAudience> lookalikeAudiences;

        private Builder() {
          // force use of static builder() method
        }

        public Builder behaviors(@Nullable List<AudienceSegment> behaviors) {
            this.behaviors = behaviors;
            return this;
        }

        public Builder combinationSpec(@Nullable List<AudienceCombination> combinationSpec) {
            this.combinationSpec = combinationSpec;
            return this;
        }

        public Builder customAudiences(@Nullable List<AudienceSegment> customAudiences) {
            this.customAudiences = customAudiences;
            return this;
        }

        public Builder excludedBehaviors(@Nullable List<AudienceSegment> excludedBehaviors) {
            this.excludedBehaviors = excludedBehaviors;
            return this;
        }

        public Builder excludedCustomAudiences(@Nullable List<AudienceSegment> excludedCustomAudiences) {
            this.excludedCustomAudiences = excludedCustomAudiences;
            return this;
        }

        public Builder excludedInterests(@Nullable List<AudienceSegment> excludedInterests) {
            this.excludedInterests = excludedInterests;
            return this;
        }

        public Builder interests(@Nullable List<AudienceSegment> interests) {
            this.interests = interests;
            return this;
        }

        public Builder lookalikeAudiences(@Nullable List<LookalikeAudience> lookalikeAudiences) {
            this.lookalikeAudiences = lookalikeAudiences;
            return this;
        }

        public PropertyAdsReportMetricsCampaignTargetingAudience build() {
            return new PropertyAdsReportMetricsCampaignTargetingAudience(
                behaviors, combinationSpec, customAudiences,
                excludedBehaviors, excludedCustomAudiences, excludedInterests,
                interests, lookalikeAudiences);
        }

    }
}
