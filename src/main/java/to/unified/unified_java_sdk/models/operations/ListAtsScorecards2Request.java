/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package to.unified.unified_java_sdk.models.operations;

import com.fasterxml.jackson.annotation.JsonCreator;
import jakarta.annotation.Nonnull;
import jakarta.annotation.Nullable;
import java.lang.Double;
import java.lang.Override;
import java.lang.String;
import java.util.List;
import java.util.Optional;
import to.unified.unified_java_sdk.utils.SpeakeasyMetadata;
import to.unified.unified_java_sdk.utils.Utils;


public class ListAtsScorecards2Request {
    /**
     * The application ID to filter by
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=application_id")
    private String applicationId;

    /**
     * The candidate ID to filter by
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=candidate_id")
    private String candidateId;

    /**
     * ID of the connection
     */
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=connection_id")
    private String connectionId;

    /**
     * Fields to return
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=fields")
    private List<ListAtsScorecards2QueryParamFields> fields;

    /**
     * The interview ID to filter by
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=interview_id")
    private String interviewId;

    /**
     * The job ID to filter by
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=job_id")
    private String jobId;


    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=limit")
    private Double limit;


    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=offset")
    private Double offset;


    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=order")
    private String order;

    /**
     * Query string to search. eg. email address or name
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=query")
    private String query;

    /**
     * Raw parameters to include in the 3rd-party request. Encoded as a URL component. eg.
     * 
     * <p>raw parameters: foo=bar&amp;zoo=bar -&gt; raw=foo%3Dbar%26zoo%3Dbar
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=raw")
    private String raw;


    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=sort")
    private String sort;

    /**
     * Return only results whose updated date is equal or greater to this value (ISO-8601 /
     * YYYY-MM-DDTHH:MM:SSZ format)
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=updated_gte")
    private String updatedGte;

    @JsonCreator
    public ListAtsScorecards2Request(
            @Nullable String applicationId,
            @Nullable String candidateId,
            @Nonnull String connectionId,
            @Nullable List<ListAtsScorecards2QueryParamFields> fields,
            @Nullable String interviewId,
            @Nullable String jobId,
            @Nullable Double limit,
            @Nullable Double offset,
            @Nullable String order,
            @Nullable String query,
            @Nullable String raw,
            @Nullable String sort,
            @Nullable String updatedGte) {
        this.applicationId = applicationId;
        this.candidateId = candidateId;
        this.connectionId = Optional.ofNullable(connectionId)
            .orElseThrow(() -> new IllegalArgumentException("connectionId cannot be null"));
        this.fields = fields;
        this.interviewId = interviewId;
        this.jobId = jobId;
        this.limit = limit;
        this.offset = offset;
        this.order = order;
        this.query = query;
        this.raw = raw;
        this.sort = sort;
        this.updatedGte = updatedGte;
    }
    
    public ListAtsScorecards2Request(
            @Nonnull String connectionId) {
        this(null, null, connectionId,
            null, null, null,
            null, null, null,
            null, null, null,
            null);
    }

    /**
     * The application ID to filter by
     */
    public Optional<String> applicationId() {
        return Optional.ofNullable(this.applicationId);
    }

    /**
     * The candidate ID to filter by
     */
    public Optional<String> candidateId() {
        return Optional.ofNullable(this.candidateId);
    }

    /**
     * ID of the connection
     */
    public String connectionId() {
        return this.connectionId;
    }

    /**
     * Fields to return
     */
    public Optional<List<ListAtsScorecards2QueryParamFields>> fields() {
        return Optional.ofNullable(this.fields);
    }

    /**
     * The interview ID to filter by
     */
    public Optional<String> interviewId() {
        return Optional.ofNullable(this.interviewId);
    }

    /**
     * The job ID to filter by
     */
    public Optional<String> jobId() {
        return Optional.ofNullable(this.jobId);
    }

    public Optional<Double> limit() {
        return Optional.ofNullable(this.limit);
    }

    public Optional<Double> offset() {
        return Optional.ofNullable(this.offset);
    }

    public Optional<String> order() {
        return Optional.ofNullable(this.order);
    }

    /**
     * Query string to search. eg. email address or name
     */
    public Optional<String> query() {
        return Optional.ofNullable(this.query);
    }

    /**
     * Raw parameters to include in the 3rd-party request. Encoded as a URL component. eg.
     * 
     * <p>raw parameters: foo=bar&amp;zoo=bar -&gt; raw=foo%3Dbar%26zoo%3Dbar
     */
    public Optional<String> raw() {
        return Optional.ofNullable(this.raw);
    }

    public Optional<String> sort() {
        return Optional.ofNullable(this.sort);
    }

    /**
     * Return only results whose updated date is equal or greater to this value (ISO-8601 /
     * YYYY-MM-DDTHH:MM:SSZ format)
     */
    public Optional<String> updatedGte() {
        return Optional.ofNullable(this.updatedGte);
    }

    public static Builder builder() {
        return new Builder();
    }


    /**
     * The application ID to filter by
     */
    public ListAtsScorecards2Request withApplicationId(@Nullable String applicationId) {
        this.applicationId = applicationId;
        return this;
    }


    /**
     * The candidate ID to filter by
     */
    public ListAtsScorecards2Request withCandidateId(@Nullable String candidateId) {
        this.candidateId = candidateId;
        return this;
    }


    /**
     * ID of the connection
     */
    public ListAtsScorecards2Request withConnectionId(@Nonnull String connectionId) {
        this.connectionId = Utils.checkNotNull(connectionId, "connectionId");
        return this;
    }


    /**
     * Fields to return
     */
    public ListAtsScorecards2Request withFields(@Nullable List<ListAtsScorecards2QueryParamFields> fields) {
        this.fields = fields;
        return this;
    }


    /**
     * The interview ID to filter by
     */
    public ListAtsScorecards2Request withInterviewId(@Nullable String interviewId) {
        this.interviewId = interviewId;
        return this;
    }


    /**
     * The job ID to filter by
     */
    public ListAtsScorecards2Request withJobId(@Nullable String jobId) {
        this.jobId = jobId;
        return this;
    }


    public ListAtsScorecards2Request withLimit(@Nullable Double limit) {
        this.limit = limit;
        return this;
    }


    public ListAtsScorecards2Request withOffset(@Nullable Double offset) {
        this.offset = offset;
        return this;
    }


    public ListAtsScorecards2Request withOrder(@Nullable String order) {
        this.order = order;
        return this;
    }


    /**
     * Query string to search. eg. email address or name
     */
    public ListAtsScorecards2Request withQuery(@Nullable String query) {
        this.query = query;
        return this;
    }


    /**
     * Raw parameters to include in the 3rd-party request. Encoded as a URL component. eg.
     * 
     * <p>raw parameters: foo=bar&amp;zoo=bar -&gt; raw=foo%3Dbar%26zoo%3Dbar
     */
    public ListAtsScorecards2Request withRaw(@Nullable String raw) {
        this.raw = raw;
        return this;
    }


    public ListAtsScorecards2Request withSort(@Nullable String sort) {
        this.sort = sort;
        return this;
    }


    /**
     * Return only results whose updated date is equal or greater to this value (ISO-8601 /
     * YYYY-MM-DDTHH:MM:SSZ format)
     */
    public ListAtsScorecards2Request withUpdatedGte(@Nullable String updatedGte) {
        this.updatedGte = updatedGte;
        return this;
    }


    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        ListAtsScorecards2Request other = (ListAtsScorecards2Request) o;
        return 
            Utils.enhancedDeepEquals(this.applicationId, other.applicationId) &&
            Utils.enhancedDeepEquals(this.candidateId, other.candidateId) &&
            Utils.enhancedDeepEquals(this.connectionId, other.connectionId) &&
            Utils.enhancedDeepEquals(this.fields, other.fields) &&
            Utils.enhancedDeepEquals(this.interviewId, other.interviewId) &&
            Utils.enhancedDeepEquals(this.jobId, other.jobId) &&
            Utils.enhancedDeepEquals(this.limit, other.limit) &&
            Utils.enhancedDeepEquals(this.offset, other.offset) &&
            Utils.enhancedDeepEquals(this.order, other.order) &&
            Utils.enhancedDeepEquals(this.query, other.query) &&
            Utils.enhancedDeepEquals(this.raw, other.raw) &&
            Utils.enhancedDeepEquals(this.sort, other.sort) &&
            Utils.enhancedDeepEquals(this.updatedGte, other.updatedGte);
    }
    
    @Override
    public int hashCode() {
        return Utils.enhancedHash(
            applicationId, candidateId, connectionId,
            fields, interviewId, jobId,
            limit, offset, order,
            query, raw, sort,
            updatedGte);
    }
    
    @Override
    public String toString() {
        return Utils.toString(ListAtsScorecards2Request.class,
                "applicationId", applicationId,
                "candidateId", candidateId,
                "connectionId", connectionId,
                "fields", fields,
                "interviewId", interviewId,
                "jobId", jobId,
                "limit", limit,
                "offset", offset,
                "order", order,
                "query", query,
                "raw", raw,
                "sort", sort,
                "updatedGte", updatedGte);
    }

    @SuppressWarnings("UnusedReturnValue")
    public final static class Builder {

        private String applicationId;

        private String candidateId;

        private String connectionId;

        private List<ListAtsScorecards2QueryParamFields> fields;

        private String interviewId;

        private String jobId;

        private Double limit;

        private Double offset;

        private String order;

        private String query;

        private String raw;

        private String sort;

        private String updatedGte;

        private Builder() {
          // force use of static builder() method
        }

        /**
         * The application ID to filter by
         */
        public Builder applicationId(@Nullable String applicationId) {
            this.applicationId = applicationId;
            return this;
        }

        /**
         * The candidate ID to filter by
         */
        public Builder candidateId(@Nullable String candidateId) {
            this.candidateId = candidateId;
            return this;
        }

        /**
         * ID of the connection
         */
        public Builder connectionId(@Nonnull String connectionId) {
            this.connectionId = Utils.checkNotNull(connectionId, "connectionId");
            return this;
        }

        /**
         * Fields to return
         */
        public Builder fields(@Nullable List<ListAtsScorecards2QueryParamFields> fields) {
            this.fields = fields;
            return this;
        }

        /**
         * The interview ID to filter by
         */
        public Builder interviewId(@Nullable String interviewId) {
            this.interviewId = interviewId;
            return this;
        }

        /**
         * The job ID to filter by
         */
        public Builder jobId(@Nullable String jobId) {
            this.jobId = jobId;
            return this;
        }

        public Builder limit(@Nullable Double limit) {
            this.limit = limit;
            return this;
        }

        public Builder offset(@Nullable Double offset) {
            this.offset = offset;
            return this;
        }

        public Builder order(@Nullable String order) {
            this.order = order;
            return this;
        }

        /**
         * Query string to search. eg. email address or name
         */
        public Builder query(@Nullable String query) {
            this.query = query;
            return this;
        }

        /**
         * Raw parameters to include in the 3rd-party request. Encoded as a URL component. eg.
         * 
         * <p>raw parameters: foo=bar&amp;zoo=bar -&gt; raw=foo%3Dbar%26zoo%3Dbar
         */
        public Builder raw(@Nullable String raw) {
            this.raw = raw;
            return this;
        }

        public Builder sort(@Nullable String sort) {
            this.sort = sort;
            return this;
        }

        /**
         * Return only results whose updated date is equal or greater to this value (ISO-8601 /
         * YYYY-MM-DDTHH:MM:SSZ format)
         */
        public Builder updatedGte(@Nullable String updatedGte) {
            this.updatedGte = updatedGte;
            return this;
        }

        public ListAtsScorecards2Request build() {
            return new ListAtsScorecards2Request(
                applicationId, candidateId, connectionId,
                fields, interviewId, jobId,
                limit, offset, order,
                query, raw, sort,
                updatedGte);
        }

    }
}
